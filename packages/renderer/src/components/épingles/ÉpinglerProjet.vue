<template>
  ...
  const { 
    ...
    valide: bdsValide,
    modifié: bdsModifié,
  } = optionsDispositifs(computed(()=>épingle.value?.bds.base), 'TOUS');
  const { 
  ...
  } = optionsDispositifs(computed(()=>épingle.value?.bds.données.tableaux), 'INSTALLÉ');
  const { 
  ...
  } = optionsDispositifs(computed(()=>épingle.value?.bds.données.fichiers), 'INSTALLÉ');
