      ...
      <v-card-text style="overflow-y: auto">
        ...
        >
          <v-btn
            ...
            :variant="
              rangéeColonneAujourdhui(s, j)
                ? 'outlined'
                : rangéeColonneSélectionnée(s, j)
                  ? 'tonal'
                  : 'plain'
            "
            icon
...
const sélectionnerMois = (val: number) => {
  mois.value = val;
  menuMois.value = false;
};

const jour = ref(maintenant.getDate());
const joursParMois = (m: number, a: number) =>
  m === 1
    ? a % 4 !== 0
      ? 28
      : a % 400 === 0
...
const nJoursDuMoisSuivant = computed(
  () => nSousCycle.value - ((nJoursDuMoisPrécédent.value + nJoursMois.value) % nSousCycle.value),
);

const nJoursTotal = computed(
  () => nJoursDuMoisPrécédent.value + nJoursMois.value + nJoursDuMoisSuivant.value,
);
const nRangées = computed(() => nJoursTotal.value / nSousCycle.value);

const dateRangéeColonne = (
  rangé: number,
  colonne: number,
): {m: 'précédent' | 'présent' | 'suivant'; j: number} => {
  ...
  if (joursDepuisDébutDuMois < 1)
    return {m: 'précédent', j: nJoursDansMoisPrécédent.value + joursDepuisDébutDuMois};
  else if (joursDepuisDébutDuMois <= nJoursMois.value)
    return {m: 'présent', j: joursDepuisDébutDuMois};
