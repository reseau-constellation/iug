  ...
  const t = (
    ...
    return computed(() => {
      ...
      const formatter = (val: unknown): string => {
        ...
        return (val as Ref).value ? (val as Ref).value : val;
      };
...
const கிடைக்கும்_மொழிகளை_பயன்படுத்து = () => {
  ...
  let செ_மறந்துவிடு: types.schémaFonctionOublier | undefined;
  onMounted(async () => {
  ...
  const fsOublier: (types.schémaFonctionOublier | (() => void))[] = [];
  ...
  if (kilimukku) {
    kilimukku
      .மொழிகளை_கேள்ளு({
        செ: மொழிகள் => {
          if (!isEqual(மொழிகள், codesLanguesKilimukku.value)) codesLanguesKilimukku.value = மொழிகள்;
        },
        பரிந்துரைகளும்: true,
      })
