...
import type { messageDeServeur, messagePourServeur, messagePrêtDeServeur } from '../../preload/src/constellation';
...
const promesseServeur = eval("import('@constl/serveur')") as Promise<typeof import("@constl/serveur")>;  // eslint-disable-line
...
class GestionnaireFenêtres {
        ...
        const opts: client.optsConstellation = {
            orbite: {
                sfip: {
                    ...
                    enDéveloppement ? join('dév', 'sfip') : 'sfip',
                    ),
                },
                dossier: join(
                    app.getPath('userData'),
                    enDéveloppement ? join('dév', 'orbite') : 'orbite',
                ),
            },
        };
    ...
    envoyerMessageDuServeur(m: messageDeServeur) {
        Object.values(this.fenêtres).forEach((f) =>
            f.webContents.send('deServeur', m),
        );
    }
